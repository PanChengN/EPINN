...
class Kovasznay_PINN:
    def __init__(self, xb, yb, ub, vb, pb, x, y, layers, model_type):
        ...
        self.Xb = torch.tensor(Xb).float().to(device)
        ...
        # Initialize the neural network layer with the DNN class
        if self.model_type in ['PINN']:
            ...
            self.sigma_r = torch.nn.Parameter(self.sigma_r)
            self.optimizer_lam = torch.optim.Adam([self.sigma_r] + [self.sigma_b], lr=1e-3)
        # Initialize lambda_1 and lambda_2 to 0 and set to require gradient computation
        self.lambda_1 = torch.tensor([0.0], requires_grad=True).to(device)
        self.lambda_2 = torch.tensor([0.0], requires_grad=True).to(device)
        # Set lambda_1 and lambda_2 as the parameters of the neural network
    ...
    def net_f_NS(self, x, y):
        ...
        f_u = self.lambda_1 * (u * u_x + v * u_y) + p_x - self.lambda_2 * (u_xx + u_yy)
        f_v = self.lambda_1 * (u * v_x + v * v_y) + p_y - self.lambda_2 * (v_xx + v_yy)
        f_e = u_x + v_y
            ...
            if it % 100 == 0:
                if self.model_type in ['PINN']:
                    pbar.set_postfix({
                        ...
                        'w_b': '{0:.2f}'.format(1.),
                        'w_r': '{0:.2f}'.format(1.),
                        'lam_1': '{0:.2f}'.format(self.lambda_1.item()),
                        'lam_2': '{0:.2f}'.format(self.lambda_2.item())
                    })
                elif self.model_type in ['EPINN-E']:
                    pbar.set_postfix({
                        'Loss': '{0:.3e}'.format(true_loss.item()),
...
if __name__ == '__main__':
    ...
    for model_type in model_list:
        model = Kovasznay_PINN(xb_train, yb_train, ub_train, vb_train, pb_train, x_train, y_train, layers, model_type)
        model.train(epoch)

        # ===================Model prediction, evaluation=============================
        num_test = 30000
        # Forecast data preparation. Also used to find exact solutions
        x_star = (np.random.rand(num_test, 1) - 1 / 3) * 3 / 2
        y_star = (np.random.rand(num_test, 1) - 1 / 4) * 2

        # The exact value of the predicted dataset for the uvp
        ...
        # Evaluate the model and calculate the relative error
        error_u = np.linalg.norm(u_star - u_pred, 2) / np.linalg.norm(u_star, 2)
        error_v = np.linalg.norm(v_star - v_pred, 2) / np.linalg.norm(v_star, 2)
        error_p = np.linalg.norm(p_star - p_pred, 2) / np.linalg.norm(p_star, 2)
        error_lambda_1 = np.abs(lambda_1_value - 1.0) * 100
        error_lambda_2 = np.abs(lambda_2_value - 0.025) / 0.025 * 100
        ...
        error_u = np.abs(u_star - u_pred)
        error_v = np.abs(v_star - v_pred)
        error_p = np.abs(p_star - p_pred)

        # Plotting the loss image
        ...
        # Plot the parameter change image
        plt.figure(figsize=(10, 6))
        ...
        # Heat maps of exact solutions, predicted solutions, and errors are plotted
        plot_heatmap(x_star.flatten(), y_star.flatten(), u_star.flatten(), 'Exact u', f'./pic/{model_type}_{layer_str}_exact_u.pdf')
        plot_heatmap(x_star.flatten(), y_star.flatten(), u_pred.flatten(), 'Predicted u', f'./pic/{model_type}_{layer_str}_predicted_u.pdf')
        plot_heatmap(x_star.flatten(), y_star.flatten(), error_u.flatten(), 'Absolute Error u', f'./pic/{model_type}_{layer_str}_error_u.pdf')

        plot_heatmap(x_star.flatten(), y_star.flatten(), v_star.flatten(), 'Exact v', f'./pic/{model_type}_{layer_str}_exact_v.pdf')
        plot_heatmap(x_star.flatten(), y_star.flatten(), v_pred.flatten(), 'Predicted v', f'./pic/{model_type}_{layer_str}_predicted_v.pdf')
        plot_heatmap(x_star.flatten(), y_star.flatten(), error_v.flatten(), 'Absolute Error v', f'./pic/{model_type}_{layer_str}_error_v.pdf')

        plot_heatmap(x_star.flatten(), y_star.flatten(), p_star.flatten(), 'Exact p', f'./pic/{model_type}_{layer_str}_exact_p.pdf')
        plot_heatmap(x_star.flatten(), y_star.flatten(), p_pred.flatten(), 'Predicted p', f'./pic/{model_type}_{layer_str}_predicted_p.pdf')
        plot_heatmap(x_star.flatten(), y_star.flatten(), error_p.flatten(), 'Absolute Error p', f'./pic/{model_type}_{layer_str}_error_p.pdf')